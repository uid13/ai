...
public class RestWebClientConfig {
     ...
     * DashScopeChatAutoConfiguration 会自动注入这个 Bean
     */
    @Bean
     ...
     */
    @Bean
    public WebClientConfigVerifier webClientConfigVerifier(WebClient.Builder webClientBuilder) {
        return new WebClientConfigVerifier(webClientBuilder);
    }

    /**
     * 调试类：验证 WebClient.Builder 的配置
     */
    static class WebClientConfigVerifier {
        public WebClientConfigVerifier(WebClient.Builder builder) {
            log.info("=== WebClient.Builder 类型：{} ===", builder.getClass().getName());
            log.info("=== 验证 WebClient 配置完成 ===");
        }
    }
